...
async def select_best_match(
    ...
    options = "\n".join(
        [
            f'''{i}. test_name: ({doc.metadata['test_name'].lower()}) 
                     test_code: ({doc.metadata['test_code'].lower()})
                     department: ({doc.metadata['department'].lower()})
                     code_letters: ({doc.metadata['code_letters'].lower()})
                     encoded_code_letters: ({doc.metadata['encoded'].lower()} 
                     biomaterial_type: ({doc.metadata['biomaterial_type'].lower()} 
                     - score: {score:.2f}  \n
            '''
    ...
    prompt = f"""
        ...
        - Совпадение ключевых терминов в названии теста и encoded_code_letters
        ...
        - Соответсвие биоматериала, если он есть в запросе
        ...
        3. **ОБЩИЕ ПРОФИЛИ** - комплексные анализы, если запрос общий
